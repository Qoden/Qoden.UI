...
namespace Qoden.View
{
	...
	{
		static readonly Dictionary<string, RGB> namedColors = new Dictionary<string, RGB>();

		// Extension for string
		public static RGB ToColor(this string value)
		{
			if (value == null)
				throw new Exception("Null cannot be converted to color");
			...
			throw new Exception(string.Format("Cannot convert {0} to color", value));
		}
		...
		private static bool IsHexColor(string value)
		{
			...
			var hexDigitsSet = new HashSet<char>("0123456789abcdefABCDEF");

			foreach (char c in value.ToCharArray())
				if (!hexDigitsSet.Contains(c))
					return false;
